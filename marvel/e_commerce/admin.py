...
class Wish_list_admin(admin.ModelAdmin):
    list_display=('user_id','comic_title')
    search_fields = ['user_id','comic_id__title']
    fields = ('user_id', 'comic_id', 'favorite','cart','wished_qty','buied_qty')
    
    #para mostrar el titilo del Comic usando la relacion FK
    @admin.display(ordering='comic_id__title')
    def comic_title(self, obj):
        return obj.comic_id.title